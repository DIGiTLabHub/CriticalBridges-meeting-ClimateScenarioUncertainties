...
\journal{Advances in Structural Engienering}
...
\hline
1 & 2.9 & Missouri River (avg. flood) & 100 & 2.68 & 1.13 & 4.81 \\
2 & 6.5 & Colorado River (2009 peak) & 500 & 4.87 & 2.06 & 8.75 \\
3 & 10.0 & Midwestern extreme (500-yr design flood) & 1000 & 6.44 & 2.72 & 11.56 \\
\hline
\end{tabular}%
}
\end{table*}

The scour statistics in Table~\ref{tab:combined_scenarios} are computed using the adopted bridge pier diameter, $d_p = 1.5$ m, and 1,000 LHS samples for each scenario.
...
\begin{equation}
\tau_{\text{max}} = 0.094 \cdot \rho_w \cdot U^2 \left[ \frac{1}{\log_{10}(Re)} - 0.1 \right]
\label{eq:shear}
\end{equation}
...
\begin{equation}
z_{50} = \frac{t_{\text{eq}}}{1/\dot{z} + t_{\text{eq}}/z_{\text{max}}}
\label{eq:z50}
\end{equation}
...
\begin{equation}
z_{50}^{\text{final}} = \frac{e^{-0.085} \cdot z_{50} \cdot e^{0.407 \epsilon}}{1000}
\label{eq:zfinal}
\end{equation}
...
The modeled bridge represents a typical Midwestern U.S. bridge: a four-span, non-skewed prestressed concrete structure, with each span measuring 35 meters and a total deck width of 12 meters. The superstructure consists of I-girders supporting a cast-in-place concrete deck, resting on elastomeric bearings over reinforced concrete piers and pile group foundations embedded in layered soil. In the model coordinate system, the original riverbed elevation is $z=-13.05$ m and the pile tips are located at $z=-33.05$ m; therefore, the modeled embedded pile length below the original riverbed is 20 m. In the following analysis, $s_z$ denotes a realized scour depth measured downward from the original riverbed, not the global $z$ coordinate. The corresponding scoured-bed elevation is $z_b=-13.05\,\mathrm{m}-s_z$; for example, $s_z=1$ m corresponds to $z_b=-14.05$ m.
...
Soil–structure interaction is modeled through nonlinear \texttt{zeroLength} springs at the pile–soil interface, simulating $p$–$y$ lateral response and $t$–$z$ vertical shaft response following FHWA HEC-18 recommendations. The lateral springs use depth-dependent \texttt{PySimple1} materials in the two horizontal directions, while the vertical shaft springs use depth-dependent \texttt{TzSimple1} materials in the vertical direction. These $p$–$y$ and $t$–$z$ springs are assigned to all 18 modeled piles across the left, center, and right pile groups at common soil-spring elevations from $z=-13.05$ m to $z=-32.05$ m: the spacing is 0.5 m from $z=-13.05$ m to $z=-23.05$ m and 1.0 m from $z=-23.05$ m to $z=-32.05$ m. Springs at or above the scoured-bed elevation $z_b$ are deactivated to simulate soil loss, thereby reducing lateral and axial stiffness. This treatment assumes a uniform scour depth across all pile groups; in an actual river channel, local hydraulics may produce non-uniform scour profiles among bents and individual piles, which can influence bridge response and is not explicitly modeled here \citep{atecs2024seismic}.

To reflect scour-induced soil loss, springs are progressively removed as $s_z$ increases. While material property uncertainties are included, they are intentionally minimized so that scour remains the dominant source of variability. Specifically, concrete compressive strength $f'_c$ is modeled as a normal distribution with a mean of 27.0 MPa and a standard deviation of 1.0 MPa. Steel yield strength $f_y$ is modeled using a lognormal distribution with a mean of 420.0 MPa and a standard deviation of 4.2 MPa.
...
This study develops data-driven surrogate models to predict structural capacity measures directly from scour depth input. The goal is to replace computationally intensive finite-element simulations with lightweight predictive models mapping a realized scour depth ($s_z$) to the four components of the capacity tuple:
