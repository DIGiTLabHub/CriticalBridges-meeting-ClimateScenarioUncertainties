...
def load_dependencies():
    global pd, np, plt, sns, joblib
    global GradientBoostingRegressor, SVR, train_test_split
    global r2_score, mean_squared_error, resample, StandardScaler
...
def load_scenario_data(
    scenario, excel_path="RecorderData/Yield_Results_by_Scenario.xlsx"
):
...
def train_model_bootstrap(
    X_train, y_train, model_type, n_bootstrap=30, random_state=42
):
    ...
    for i in range(n_bootstrap):
        X_res, y_res = resample(
            X_train, y_train, replace=True, random_state=random_state + i
        )
...
def plot_capacity_trajectory(df, credal_bounds, target_pairs, output_dir, scenario):
    ...
    for idx, (x_target, y_target) in enumerate(target_pairs):
        ...
        ax.set_xlabel(
            f"{x_target} ({'kN' if x_target == 'Vy' else 'mm' if x_target == 'Dy' else 'kNm' if x_target == 'My' else 'rad'})"
        )
        ax.set_ylabel(
            f"{y_target} ({'kN' if y_target == 'Vy' else 'mm' if y_target == 'Dy' else 'kNm' if y_target == 'My' else 'rad'})"
        )
    ...
    plt.tight_layout()
    plt.savefig(
        output_dir / f"capacity_trajectories_{scenario}.png",
        dpi=300,
        bbox_inches="tight",
    )
    plt.close()
...
def plot_distribution_histograms(df, critical_points, output_dir, scenario):
    ...
    for point in critical_points:
        ...
        plt.suptitle(
            f"Distribution Analysis at Scour = {point['scour']:.1f} mm - {scenario}"
        )
        plt.tight_layout()
        plt.savefig(
            output_dir / f"distributions_scour_{point['scour']:.0f}mm_{scenario}.png",
            dpi=300,
            bbox_inches="tight",
        )
        plt.close()
...
def main():
    ...
    parser.add_argument(
        "--plots", action="store_true", help="Generate distribution plots"
    )
    parser.add_argument(
        "--output-dir",
        default="experiments/test_output",
    ...
    for target_idx, target in enumerate(targets):
        ...
        for method in methods:
            ...
            print(
                f"    ✅ R² = {r2:.3f}, RMSE = {rmse:.3f}, Credal Width = {np.mean(upper - lower):.3f}"
            )
    ...
    perf_df.to_excel(
        output_dir / f"model_performance_{args.scenario}.xlsx", index=False
    )
    ...
    if args.plots:
        ...
        plot_capacity_trajectory(
            df, None, [("Vy", "Dy"), ("My", "Thy")], output_dir, args.scenario
        )
    ...
    critical_df.to_excel(
        output_dir / f"critical_points_{args.scenario}.xlsx", index=False
    )
    ...
    if args.plots:
        print(f"  - capacity_trajectories_{args.scenario}.png")
        print("  - distribution histograms for critical points")
